...
export default function JobProgressCard({ job }: Props) {
  const getJobStepStatus = (job: Job) => {
    // 1. Job just created
    if (job.status === 'created') {
      return 'created';
    }

    // 2. Provider is assigned / working
    if (job.status === 'in-progress' && !job.providerDone) {
      return 'in-progress';
    }

    // 3. Provider finished the job (customer can now confirm)
    if (job.providerDone) {
      return 'completed';
    }
  ...
  // const currentStepIndex = STATUS_STEPS.findIndex((step) => step.key === job.status);
  const currentStepIndex = STATUS_STEPS.findIndex((step) => step.key === getJobStepStatus(job));
